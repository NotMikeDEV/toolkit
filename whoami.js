...
module.exports = {
	HTTP: function(req, res, startTime, URL, Path, bodyFilename, bodyLength, SendReply) {
		...
		try {
			...
			var IPAddress = req.headers['cf-connecting-ip']?req.headers['cf-connecting-ip']:req.headers['x-real-ip'];
			if (IPAddress.substr(0,7) == '::ffff:')
				IPAddress = IPAddress.substr(7);
			return execBG('dig -x ' + IPAddress + ' +short|head -n 1', function(err, stdout){
				var Reply = "<p><b>Closest Server:</b> " + fs.readFileSync("/etc/hostname") + "</br>";
